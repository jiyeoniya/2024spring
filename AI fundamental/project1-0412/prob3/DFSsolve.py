...
def visualize_maze_with_paths(maze, explored_path, shortest_path):
    ...
    if explored_path:
        ...
        plt.plot(explored_path_y, explored_path_x, marker='o', markersize=8, color='blue', linestyle='dashed', linewidth=2, label='Explored Path')

    if shortest_path:
        shortest_path_x, shortest_path_y = zip(*shortest_path)
        plt.plot(shortest_path_y, shortest_path_x, marker='o', markersize=8, color='red', linewidth=3, label='Shortest Path')
...
# 使用DFS算法求解最短路径
shortest_path, min_steps = dfs(maze, start, end)
